...
$productoId  = isset($body['productoId'])  && $body['productoId']  !== '' ? intval($body['productoId'])  : null;
$origen      = isset($body['origen'])      && $body['origen']      !== '' ? $body['origen']              : null;

$sql    = "{CALL sp_ReporteSalidas(?, ?, ?, ?, ?)}";
$params = [
    [$fechaDesde, SQLSRV_PARAM_IN],
    [$fechaHasta, SQLSRV_PARAM_IN],
    [$sucursalId, SQLSRV_PARAM_IN],
    [$productoId, SQLSRV_PARAM_IN],
    [$origen,     SQLSRV_PARAM_IN],
];
...
ob_clean();
echo json_encode(["status" => 1, "salidas" => $salidas]);
?>