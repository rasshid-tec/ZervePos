<template>
  <div class="ticket-overlay" @click.self="$emit('cerrar')">
    <div class="ticket-modal">
      ...
      <div v-else-if="ticket" class="ticket" id="ticket-entrada-imprimir">
        <header class="ticket-header">
          <h1>{{ ticket.encabezado.NombreSucursal }}</h1>
          <p>Tel: {{ ticket.encabezado.TelefonoSucursal }}</p>
          <p class="tipo-badge">ENTRADA — {{ ticket.encabezado.Tipo }}</p>
        </header>

        <div class="ticket-info">
          <div><span>Folio:</span> #{{ ticket.encabezado.EntradaId }}</div>
...
const props = defineProps({
  entradaId: { type: [Number, String], required: true },
});
defineEmits(['cerrar']);
...
async function cargarTicket() {
  try {
    const res  = await fetch('/php/tickets/ticket_entrada.php', {
      ...
      body: JSON.stringify({ entradaId: props.entradaId }),
    });
    const data = await res.json();
    if (data.success && data.encabezado?.EntradaId) {
      ticket.value = data;
    } else {
      error.value = 'No se encontró la entrada #' + props.entradaId;
    }
  } catch (e) {
...
function imprimir() {
  const contenido = document.getElementById('ticket-entrada-imprimir');
  if (!contenido) return;
  const ventana = window.open('', '', 'width=400,height=600');
  ventana.document.write(`
    <html>
      <head>
        <title>Entrada #${props.entradaId}</title>
        <style>
...
.ticket-modal {
  ...
  max-width: 380px; width: 90%; max-height: 90vh;
...
.tipo-badge { font-weight: 700; font-size: 13px !important; margin-top: 6px !important; }
.ticket-info {
...
.ticket-footer { text-align: center; margin-top: 10px; border-top: 1px dashed #000; padding-top: 6px; font-size: 11px; color: #64748b; }
