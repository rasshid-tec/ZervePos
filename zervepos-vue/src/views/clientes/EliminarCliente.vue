<template>
  <div class="eliminar-cliente">
    ...
    <div class="page-header">
      <div class="page-header__left">
        ...
        <div>
          ...
          <h1 class="page-title">Eliminar Cliente</h1>
        </div>
      </div>
    </div>
    ...
    <div v-if="!clienteSeleccionado" class="empty-state">
      ...
      <p class="empty-state__desc">Usa el buscador para encontrar el cliente que deseas eliminar.</p>
    </div>

    <div v-else class="cliente-card">
      <div class="cliente-card__header">
        <div class="cliente-card__icono">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        </div>
        <div>
          <h2 class="cliente-card__nombre">{{ clienteSeleccionado.NombreCliente }}</h2>
          <p class="cliente-card__sub">{{ clienteSeleccionado.Negocio }} · {{ clienteSeleccionado.NombreCategoria || 'Sin categoría' }}</p>
        </div>
      </div>

      <div class="cliente-card__detalle">
        <div class="detalle-row"><span class="detalle-row__label">Teléfono</span><span class="detalle-row__value">{{ clienteSeleccionado.Telefono }}</span></div>
        <div class="detalle-row"><span class="detalle-row__label">Correo</span><span class="detalle-row__value">{{ clienteSeleccionado.Correo || '—' }}</span></div>
        <div class="detalle-row"><span class="detalle-row__label">Ciudad</span><span class="detalle-row__value">{{ clienteSeleccionado.Ciudad || '—' }}</span></div>
        <div class="detalle-row"><span class="detalle-row__label">Dirección</span><span class="detalle-row__value">{{ clienteSeleccionado.Direccion || '—' }}</span></div>
        <div class="detalle-row"><span class="detalle-row__label">Crédito</span><span class="detalle-row__value">${{ clienteSeleccionado.CreditoPermitido }}</span></div>
        <div class="detalle-row"><span class="detalle-row__label">Categoría</span><span class="detalle-row__value">{{ clienteSeleccionado.NombreCategoria || '—' }}</span></div>
      </div>

      <div class="advertencia">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
        <p>Este cliente será desactivado del sistema. No aparecerá en ventas ni búsquedas, pero sus registros históricos se conservarán.</p>
      </div>

      <div class="cliente-card__acciones">
        <button class="btn btn--secondary" @click="cancelar">Cancelar</button>
        <button class="btn btn--danger" @click="eliminarCliente" :disabled="eliminando">
...
const eliminarCliente = async () => {
  const confirmado = await confirmacion.value.mostrar(
    '¿Eliminar cliente?',
    `"${clienteSeleccionado.value.NombreCliente}" será desactivado del sistema. Esta acción requiere confirmación.`
  )
  ...
  try {
    ...
    const data = await res.json()
    if      (data.status === 1)  { alerta.value.mostrar('Cliente eliminado correctamente', 'exito'); cancelar() }
    else if (data.status === -2) { alerta.value.mostrar('El cliente no existe o ya fue eliminado', 'advertencia') }
    else                         { alerta.value.mostrar('Error al eliminar el cliente', 'error') }
  } catch (e) {
...
<style scoped>
.eliminar-cliente { padding: 1.5rem; max-width: 800px; margin: 0 auto; }
...
.cliente-card { background: #fff; border-radius: 12px; border: 1px solid #fecaca; overflow: hidden; }
.cliente-card__header { display: flex; align-items: center; gap: 1rem; padding: 1.5rem; border-bottom: 1px solid #fee2e2; }
.cliente-card__icono { width: 52px; height: 52px; background: #fff1f1; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #dc2626; flex-shrink: 0; }
.cliente-card__icono svg { width: 26px; height: 26px; }
.cliente-card__nombre { font-size: 1.1rem; font-weight: 700; color: #1a2b3c; margin: 0 0 0.2rem; }
.cliente-card__sub { font-size: 0.85rem; color: #94a3b8; margin: 0; }
.cliente-card__detalle { padding: 1.25rem 1.5rem; display: grid; grid-template-columns: 1fr 1fr; gap: 0; }
...
.cliente-card__acciones { display: flex; gap: 0.75rem; justify-content: flex-end; padding: 1rem 1.5rem; border-top: 1px solid #fee2e2; }
