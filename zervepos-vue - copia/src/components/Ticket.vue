<template>
  <div class="ticket-overlay" @click.self="$emit('cerrar')">
    <div class="ticket-modal">
      ...
      <div v-else-if="ticket" class="ticket" id="ticket-imprimir">
        ...
        <div class="ticket-info">
          ...
          <div><span>Fecha:</span> {{ ticket.encabezado.FechaVenta }}</div>
...
async function cargarTicket() {
  try {
    const data = await api.obtenerTicket(props.ventaId);
    if (data.success) {
      ticket.value = data;
...
function imprimir() {
  const contenido = document.getElementById('ticket-imprimir');
  if (!contenido) return;
...
.btn-imprimir, .btn-cerrar { flex: 1; padding: 10px; border: 0; border-radius: 8px; cursor: pointer; font-size: 14px; }
